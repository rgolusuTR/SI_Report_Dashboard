...
export const useReportData = () => {
  const [reportData, setReportData] = useState<ReportData[]>(generateDummyData());
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>(dummyUploadedFiles);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const uploadFile = useCallback(async (file: File, website: string, reportType: string) => {
    console.log('useReportData.uploadFile called:', { fileName: file.name, website, reportType });
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      console.log('File processed successfully:', { dataLength: data.length, metadata });
  ...
  const filterData = useCallback((filters: FilterState): ReportData[] => {
