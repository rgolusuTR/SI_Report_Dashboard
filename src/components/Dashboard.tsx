...
export const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  ...
  const filteredData = data.filter(item => {
    // Website filter
    if (filters.websites.length > 0 && !filters.websites.includes(item.website)) {
      return false;
    }
  ...
  const summaryStats = {
    'Total Misspellings': filteredData.filter(item => 
      item.id.startsWith('misspellings') || item.id.startsWith('misspelling-history')
    ).length,
          ...
          <div key={key} className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="h-12 w-12 bg-blue-100 rounded-lg flex items-center justify-center">
                {key.includes('Misspellings') && <AlertCircle className="h-6 w-6 text-blue-600" />}
                {key.includes('Review') && <FileText className="h-6 w-6 text-blue-600" />}
                {key.includes('Pages') && <BarChart className="h-6 w-6 text-blue-600" />}
                {key.includes('Websites') && <TrendingUp className="h-6 w-6 text-blue-600" />}
              </div>
            </div>
          </div>
        ))}
